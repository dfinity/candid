...
impl IDLProg {
    fn find_type(&self, id: &str) -> Result<IDLType, String> {
        ...
        Err(format!("cannot find variable {}", id))
    }
    fn as_service(&self, t: &IDLType) -> Result<IDLType, String> {
        match t {
            IDLType::ServT(_) => Ok(t.clone()),
            IDLType::VarT(id) => self.as_service(&self.find_type(id)?),
            _ => Err("as_service failed".to_owned()),
        }
    }
    fn as_func(&self, t: &IDLType) -> Result<FuncType, String> {
        match t {
            IDLType::FuncT(func) => Ok(func.clone()),
            IDLType::VarT(id) => self.as_func(&self.find_type(id)?),
            _ => Err("as_func failed".to_owned()),
        }
    }
...
impl std::str::FromStr for IDLProg {
    type Err = crate::value::ParserError;
