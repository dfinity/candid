...
Candid is designed to support higher-order use cases, where a service may receive or provide references to other serivces and their methods. The `func` type lies at the code or this: It indicates the function’s _signature_ (argument and results types, annotations), and values of this type are references to functions with that signature.
...
+
* `query` indicates that the referenced function is a query method, meaning it can does not alter the state of its canister, and that it can be invoked using the cheaper “query call” mechanism.
 ...
 * Existing parameter types may be change to to a supertype.
 * Existing result types may be _changed_ to a subtype(!).
 ...
 * Existing parameter types may be change to to a subtype.
 * Existing result types may be changed to a supertype(!).

Corresponding Motoko type::

Candid function types correspond to `shared` Motoko functions, with the result type wrapped in `async`. Arguments resp. results become tuples, unless there is exactly one, in which case it is used directly:
+
....
type F0 = func () -> ();
