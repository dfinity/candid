...
+
For example, the full URL should look similar to the following but with the `+CANDID-UI_IDNETIFIER+` that was returned by the `+dfx canister id+` command:
+
....
http://127.0.0.1:8000/candid?canisterId=<CANDID-UI-IDENTIFIER>
....
+
The browser displays a form ior you to specify a canister identifier or choose a Candid description (`+.did+`) file.
+

image:candid-ui-select-id.png[]
+
If you aren't sure of the canister identifier to use, you can run the `+dfx canister id+` command to look up the identifier for a specific canister name. 
+
