...
In other languages, like Rust, you can still develop your service using the types that are native to that language, for example, using native Rust types.
...
Therefore, you currently would need to write the Candid interface description manually.
...
In the xref:candid-types{outfilesuffix}[reference] section, you'll find Candid type mapping described for Motoko, Rust, and JavaScript.
