...
The `+blob+` type can be used for binary data, that is, sequences of bytes. It is interchangable with `+vec nat8+`. Therefore, interfaces written in terms of `+blob+` and interfaces written in terms of `+vec nat8+` are compatible.
