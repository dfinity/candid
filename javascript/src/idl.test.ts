...
test("IDL encoding (magic number)", () => {
  ...
  expect(() => IDL.decode([IDL.Nat], Buffer.from("4449444d2a"))).toThrow(
    /Wrong magic number:/
  );
});
...
test("IDL encoding (empty)", () => {
  // Empty
  expect(() => IDL.encode([IDL.Empty], [undefined])).toThrow(
    /Invalid empty argument:/
  );
  expect(() =>
    IDL.decode([IDL.Empty], Buffer.from("4449444c00016f", "hex"))
  ).toThrow(/Empty cannot appear as an output/);
});
...
test("IDL encoding (text)", () => {
  ...
  expect(() =>
    IDL.decode(
      [IDL.Vec(IDL.Nat8)],
      Buffer.from("4449444c00017107486920e298830a", "hex")
    )
...
test("IDL encoding (int)", () => {
  ...
  expect(() =>
    IDL.decode([IDL.Int], Buffer.from("4449444c00017d2a", "hex"))
  ).toThrow(/type mismatch: type on the wire nat, expect type int/);
});
...
test("IDL encoding (float64)", () => {
  ...
  test_(
    ...
    "4449444c000172ffffffffffff3fc3",
    "min_safe_integer"
  );
  test_(
    IDL.Float64,
...
test("IDL encoding (fixed-width number)", () => {
  ...
  expect(() => IDL.encode([IDL.Nat32], [-42])).toThrow(
    /Invalid nat32 argument/
  );
  expect(() => IDL.encode([IDL.Int8], [256])).toThrow(/Invalid int8 argument/);
  expect(() => IDL.encode([IDL.Int32], [0xffffffff])).toThrow(
    /Invalid int32 argument/
  );
});
...
test("IDL encoding (array)", () => {
  ...
  expect(() => IDL.encode([IDL.Vec(IDL.Int)], [["fail"]])).toThrow(
    /Invalid vec int argument/
  );
});
...
test("IDL encoding (bool)", () => {
  ...
  expect(() => IDL.encode([IDL.Bool], ["false"])).toThrow(
...
test("IDL encoding (principal)", () => {
  ...
  expect(() =>
    IDL.decode([IDL.Principal], Buffer.from("4449444c00016803caffee", "hex"))
  ).toThrow(/Cannot decode principal/);
});
...
test("IDL encoding (variants)", () => {
  ...
  expect(() => IDL.encode([Result], [{}])).toThrow(
...
test("IDL encoding (rec)", () => {
  ...
  const List = IDL.Rec();
  expect(() => IDL.encode([List], [[]])).toThrow(
    /Recursive type uninitialized/
  );
