...
impl std::str::FromStr for IDLType {
    ...
    fn from_str(str: &str) -> error::Result<Self> {
        let trivia = crate::token::TriviaMap::default();
        let lexer = crate::token::Tokenizer::new_with_trivia(str, trivia.clone());
        Ok(crate::grammar::TypParser::new().parse(Some(&trivia), lexer)?)
    }
}
...
impl std::str::FromStr for IDLTypes {
    ...
    fn from_str(str: &str) -> error::Result<Self> {
        let trivia = crate::token::TriviaMap::default();
        let lexer = crate::token::Tokenizer::new_with_trivia(str, trivia.clone());
        Ok(crate::grammar::TypsParser::new().parse(Some(&trivia), lexer)?)
    }
}
...
impl IDLProg {
    ...
    {
        crate::grammar::IDLProgParser::new().parse(trivia, tokens)
    }
    ...
    where
        I: IntoIterator<Item = std::result::Result<(usize, Token, usize), LexicalError>>,
    {
        match crate::grammar::IDLProgLossyParser::new().parse(trivia, tokens) {
            Ok(result) => result,
...
impl std::str::FromStr for IDLInitArgs {
    ...
    fn from_str(str: &str) -> error::Result<Self> {
        let trivia = crate::token::TriviaMap::default();
        let lexer = crate::token::Tokenizer::new_with_trivia(str, trivia.clone());
        Ok(crate::grammar::IDLInitArgsParser::new().parse(Some(&trivia), lexer)?)
    }
}
